...
volatile long int pos_encoder=0;
// 11 PPR * 40 Reducción = 440
const float PPR_TOTAL = 845.0;
...
void encoder_init()
{
    ...
    attachInterrupt(digitalPinToInterrupt(pin_interrupcion_A),manejo_interrupcion, RISING);//Cuando A tenga flanco de subida
}

void manejo_interrupcion()//Funcion de la interrupción
{
...
void print_encoder()
{
    ...
    noInterrupts();
    long int pasos_actuales = pos_encoder;
    interrupts();
